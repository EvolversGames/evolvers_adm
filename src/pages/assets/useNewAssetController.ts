...
export function useNewAssetController() {
  ...
  // ✅ Helper: decide qual draft usar (override ou state)
  function resolveDraft(override?: AssetDraft): AssetDraft {
    return override ?? draft;
  }

  // ✅ Publicar novo asset (aceita overrideDraft)
  async function publish(overrideDraft?: AssetDraft): Promise<PublishResult> {
    setSubmitAttempted(true);

    const payloadDraft = resolveDraft(overrideDraft);
    const errors = validateAssetDraft(payloadDraft);
    ...
    setIsSaving(true);
    try {
      const res = await assetRepository.publishDraft(payloadDraft);
      // mantém state sincronizado com o que foi enviado
  ...
  // ✅ Atualizar asset existente (aceita overrideDraft)
  async function update(id: number, overrideDraft?: AssetDraft): Promise<PublishResult> {
    setSubmitAttempted(true);
    ...
    setIsSaving(true);
    try {
      const res = await assetRepository.updateAsset(id, payloadDraft);
      // mantém state sincronizado com o que foi enviado
      if (overrideDraft) setDraft(overrideDraft);
      return { ok: true, res };
    } catch (e: any) {
